...
def load_conversation(thread_id):
    state = chatbot.get_state(config={'configurable': {'thread_id': thread_id}})  # Retrieve the state for the specific thread
    return state.values.get('messages', [])                                       # Return the messages or an empty list if not found
